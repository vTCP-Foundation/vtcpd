    ...
    mLog(logger)
{
    ...
    if (dbConnection == nullptr) {
        throw ValueError("IncomingPaymentReceiptHandler::constructor: Database connection cannot be null.");
    }

    if (tableName.empty()) {
        throw ValueError("IncomingPaymentReceiptHandler::constructor: Table name cannot be empty.");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("IncomingPaymentReceiptHandler::constructor: Failed to create table '" + mTableName + "'. "
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("IncomingPaymentReceiptHandler::constructor: Failed to create unique index on table '" + mTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("IncomingPaymentReceiptHandler::constructor: Failed to create transaction_uuid index on table '" + mTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    const Signature::Shared contractorSignature)
{
    if (!contractorPublicKeyHash) {
        throw ValueError("IncomingPaymentReceiptHandler::saveRecord: Contractor public key hash cannot be null.");
    }

    if (!contractorSignature) {
        throw ValueError("IncomingPaymentReceiptHandler::saveRecord: Contractor signature cannot be null.");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::saveRecord: Failed to bind trust_line_id. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::saveRecord: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::saveRecord: Failed to bind transaction_uuid. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::saveRecord: Failed to bind contractor_public_key_hash. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::saveRecord: Failed to bind amount. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::saveRecord: Failed to bind contractor_signature. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("IncomingPaymentReceiptHandler::saveRecord: Failed to execute INSERT. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    const AuditNumber auditNumber)
{
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::auditAmounts: Failed to bind trust_line_id. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 2, auditNumber);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::auditAmounts: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    const AuditNumber auditNumber)
{
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::receiptsByAuditNumber: Failed to bind trust_line_id. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 2, auditNumber);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::receiptsByAuditNumber: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    const AuditNumber auditNumber)
{
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::receiptsLessEqualThanAuditNumber: Failed to bind trust_line_id. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 2, auditNumber);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::receiptsLessEqualThanAuditNumber: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    const AuditNumber auditNumber)
{
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::countReceiptsByNumber: Failed to bind trust_line_id. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 2, auditNumber);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::countReceiptsByNumber: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_step(stmt.get());
    if (rc != SQLITE_ROW) {
        throw IOError("IncomingPaymentReceiptHandler::countReceiptsByNumber: Failed to execute count query. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    const TransactionUUID &transactionUUID)
{
    ...
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::deleteRecords: Failed to bind transaction_uuid. "
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }

    rc = sqlite3_step(stmt.get());
    if (rc != SQLITE_DONE) {
        throw IOError("IncomingPaymentReceiptHandler::deleteRecords: Failed to execute DELETE. "
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    const TrustLineID trustLineID)
{
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::deleteRecords: Failed to bind trust_line_id. "
...
{
    if (!keyHash) {
        throw ValueError("IncomingPaymentReceiptHandler::deleteRecords: Key hash cannot be null.");
    }
    ...
    int rc = sqlite3_bind_blob(stmt.get(), 1, keyHash->data(),
                               (int)KeyHash::kBytesSize, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::deleteRecords: Failed to bind contractor_public_key_hash. "
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
{
    if (!keyHash) {
        throw ValueError("IncomingPaymentReceiptHandler::isContainsKeyHash: Key hash cannot be null.");
    }
    ...
    int rc = sqlite3_bind_blob(stmt.get(), 1, keyHash->data(),
                               (int)KeyHash::kBytesSize, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::isContainsKeyHash: Failed to bind contractor_public_key_hash. "
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    const TransactionUUID &transactionUUID) const
{
    ...
    int rc = sqlite3_bind_blob(stmt.get(), 1, transactionUUID.data, TransactionUUID::kBytesSize, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("IncomingPaymentReceiptHandler::isContainsTransaction: Failed to bind transaction_uuid. "
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
