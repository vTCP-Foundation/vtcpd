...
HistoryStorageSQLite::HistoryStorageSQLite(
    ...
    mLog(logger)
{
    ...
    if (dbConnection == nullptr) {
        throw ValueError("HistoryStorageSQLite::constructor: Database connection cannot be null.");
    }

    if (mainTableName.empty()) {
        throw ValueError("HistoryStorageSQLite::constructor: Main table name cannot be empty.");
    }

    if (additionalTableName.empty()) {
        throw ValueError("HistoryStorageSQLite::constructor: Additional table name cannot be empty.");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create main table '" + mMainTableName + "'. "
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create operation_uuid index on main table '" + mMainTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create operation_timestamp index on main table '" + mMainTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create record_type index on main table '" + mMainTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create command_uuid index on main table '" + mMainTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create equivalent index on main table '" + mMainTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create additional table '" + mAdditionalTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create operation_uuid index on additional table '" + mAdditionalTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create operation_timestamp index on additional table '" + mAdditionalTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create record_type index on additional table '" + mAdditionalTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::constructor: Failed to create equivalent index on additional table '" + mAdditionalTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
void HistoryStorageSQLite::saveTrustLineRecord(
    TrustLineRecord::Shared record,
    const SerializedEquivalent equivalent)
{
    if (!record) {
        throw ValueError("HistoryStorageSQLite::saveTrustLineRecord: Record cannot be null.");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::saveTrustLineRecord: Failed to bind operation_uuid. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::saveTrustLineRecord: Failed to bind operation_timestamp. "
                      "Equivalent=" + to_string(equivalent) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::saveTrustLineRecord: Failed to bind equivalent. "
                      "Equivalent=" + to_string(equivalent) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::saveTrustLineRecord: Failed to bind record_type. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::saveTrustLineRecord: Failed to bind record_body. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::saveTrustLineRecord: Failed to bind record_body_bytes_count. "
                      "Equivalent=" + to_string(equivalent) + ", BodySize=" + to_string(serializedTrustLineRecordAndSize.second) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::saveTrustLineRecord: Failed to execute INSERT. "
                      "Equivalent=" + to_string(equivalent) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
void HistoryStorageSQLite::savePaymentRecord(
    PaymentRecord::Shared record)
{
    if (!record) {
        throw ValueError("HistoryStorageSQLite::savePaymentRecord: Record cannot be null.");
    }
    ...
    case PaymentRecord::IncomingPaymentType:
        ...
        break;
    default:
        throw ValueError("HistoryStorageSQLite::savePaymentRecord: Invalid payment operation type=" +
                         to_string(record->paymentOperationType()) + ".");
    }
}

void HistoryStorageSQLite::savePaymentMainOutgoingRecord(
    PaymentRecord::Shared record,
    const SerializedEquivalent equivalent)
{
    if (!record) {
        throw ValueError("HistoryStorageSQLite::savePaymentMainOutgoingRecord: Record cannot be null.");
    }
    ...
    int rc = sqlite3_bind_blob(stmt.get(), 1, record->operationUUID().data, TransactionUUID::kBytesSize, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainOutgoingRecord: Failed to bind operation_uuid. "
    ...
    rc = sqlite3_bind_int64(stmt.get(), 2, timestamp);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainOutgoingRecord: Failed to bind operation_timestamp. "
                      "Equivalent=" + to_string(record->equivalent()) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainOutgoingRecord: Failed to bind equivalent. "
                      "Equivalent=" + to_string(record->equivalent()) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 4, record->recordType());
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainOutgoingRecord: Failed to bind record_type. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainOutgoingRecord: Failed to bind record_body. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainOutgoingRecord: Failed to bind record_body_bytes_count. "
                      "Equivalent=" + to_string(record->equivalent()) + ", BodySize=" + to_string(serializedPaymentRecordAndSize.second) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainOutgoingRecord: Failed to bind command_uuid. "
                      "Equivalent=" + to_string(record->equivalent()) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_step(stmt.get());
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::savePaymentMainOutgoingRecord: Failed to execute INSERT. "
                      "Equivalent=" + to_string(record->equivalent()) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
void HistoryStorageSQLite::savePaymentMainIncomingRecord(
    PaymentRecord::Shared record,
    const SerializedEquivalent equivalent)
{
    if (!record) {
        throw ValueError("HistoryStorageSQLite::savePaymentMainIncomingRecord: Record cannot be null.");
    }
    ...
    int rc = sqlite3_bind_blob(stmt.get(), 1, record->operationUUID().data, TransactionUUID::kBytesSize, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainIncomingRecord: Failed to bind operation_uuid. "
                      "Equivalent=" + to_string(record->equivalent()) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int64(stmt.get(), 2, timestamp);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainIncomingRecord: Failed to bind operation_timestamp. "
                      "Equivalent=" + to_string(record->equivalent()) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 3, record->equivalent());
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainIncomingRecord: Failed to bind equivalent. "
                      "Equivalent=" + to_string(record->equivalent()) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 4, record->recordType());
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainIncomingRecord: Failed to bind record_type. "
                      "Equivalent=" + to_string(record->equivalent()) + ", RecordType=" + to_string(record->recordType()) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_blob(stmt.get(), 5, serializedPaymentRecordAndSize.first.get(),
                           (int) serializedPaymentRecordAndSize.second, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainIncomingRecord: Failed to bind record_body. "
                      "Equivalent=" + to_string(record->equivalent()) + ", BodySize=" + to_string(serializedPaymentRecordAndSize.second) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 6, (int) serializedPaymentRecordAndSize.second);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentMainIncomingRecord: Failed to bind record_body_bytes_count. "
                      "Equivalent=" + to_string(record->equivalent()) + ", BodySize=" + to_string(serializedPaymentRecordAndSize.second) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_step(stmt.get());
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::savePaymentMainIncomingRecord: Failed to execute INSERT. "
                      "Equivalent=" + to_string(record->equivalent()) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
void HistoryStorageSQLite::savePaymentAdditionalRecord(
    PaymentAdditionalRecord::Shared record,
    const SerializedEquivalent equivalent)
{
    if (!record) {
        throw ValueError("HistoryStorageSQLite::savePaymentAdditionalRecord: Record cannot be null.");
    }
    ...
    int rc = sqlite3_bind_blob(stmt.get(), 1, record->operationUUID().data, TransactionUUID::kBytesSize, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentAdditionalRecord: Failed to bind operation_uuid. "
                      "Equivalent=" + to_string(equivalent) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int64(stmt.get(), 2, timestamp);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentAdditionalRecord: Failed to bind operation_timestamp. "
                      "Equivalent=" + to_string(equivalent) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 3, equivalent);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentAdditionalRecord: Failed to bind equivalent. "
                      "Equivalent=" + to_string(equivalent) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 4, record->recordType());
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentAdditionalRecord: Failed to bind record_type. "
                      "Equivalent=" + to_string(equivalent) + ", RecordType=" + to_string(record->recordType()) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_blob(stmt.get(), 5, serializedPaymentRecordAndSize.first.get(),
                           (int) serializedPaymentRecordAndSize.second, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentAdditionalRecord: Failed to bind record_body. "
    ...
    rc = sqlite3_bind_int(stmt.get(), 6, (int) serializedPaymentRecordAndSize.second);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::savePaymentAdditionalRecord: Failed to bind record_body_bytes_count. "
                      "Equivalent=" + to_string(equivalent) + ", BodySize=" + to_string(serializedPaymentRecordAndSize.second) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_step(stmt.get());
    if (rc != SQLITE_DONE) {
        throw IOError("HistoryStorageSQLite::savePaymentAdditionalRecord: Failed to execute INSERT. "
                      "Equivalent=" + to_string(equivalent) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
vector<TrustLineRecord::Shared> HistoryStorageSQLite::allTrustLineRecords(
    const SerializedEquivalent equivalent,
    size_t recordsCount,
    size_t fromRecord,
    DateTime timeFrom,
    ...
    bool isTimeToPresent)
{
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::allTrustLineRecords: Failed to bind equivalent. "
                      "Equivalent=" + to_string(equivalent) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::allTrustLineRecords: Failed to bind record_type. "
                      "Equivalent=" + to_string(equivalent) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (isTimeFromPresent) {
        ...
        if (rc != SQLITE_OK) {
            throw IOError("HistoryStorageSQLite::allTrustLineRecords: Failed to bind time_from. "
    ...
    if (isTimeToPresent) {
        ...
        rc = sqlite3_bind_int64(stmt.get(), idxParam++, timestamp);
        if (rc != SQLITE_OK) {
            throw IOError("HistoryStorageSQLite::allTrustLineRecords: Failed to bind time_to. "
                          "Equivalent=" + to_string(equivalent) + ", Timestamp=" + to_string(timestamp) +
                          ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
        }
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::allTrustLineRecords: Failed to bind records_count. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::allTrustLineRecords: Failed to bind from_record. "
...
vector<PaymentRecord::Shared> HistoryStorageSQLite::allPaymentRecordsBase(
    const SerializedEquivalent equivalent,
    size_t recordsCount,
    size_t fromRecord,
    DateTime timeFrom,
    ...
    bool isTimeToPresent)
{
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::allPaymentRecords: "
...
vector<PaymentRecord::Shared> HistoryStorageSQLite::paymentRecordsAllEquivalentsBase(
    size_t recordsCount,
    size_t fromRecord,
    DateTime timeFrom,
    bool isTimeFromPresent,
    ...
    bool isTimeToPresent)
{
    ...
    int rc = sqlite3_prepare_v2(mDataBase, query.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::paymentRecordsAllEquivalents: "
                      "Bad query; sqlite error: " + to_string(rc));
    }
    int idxParam = 1;
    rc = sqlite3_bind_int(stmt, idxParam++, Record::PaymentRecordType);
...
size_t HistoryStorageSQLite::countRecordsByType(
    ...
    const SerializedEquivalent equivalent)
{
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::countRecordsByType: Failed to bind equivalent. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::countRecordsByType: Failed to bind record_type. "
                      "Equivalent=" + to_string(equivalent) + ", RecordType=" + to_string(recordType) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_step(stmt.get());
    if (rc != SQLITE_ROW) {
        throw IOError("HistoryStorageSQLite::countRecordsByType: Failed to execute count query. "
                      "Equivalent=" + to_string(equivalent) + ", RecordType=" + to_string(recordType) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
size_t HistoryStorageSQLite::countRecordsByTypeAllEquivalents(
    Record::RecordType recordType)
{
    string query = "SELECT count(*) FROM "
                   + mMainTableName + " WHERE record_type = ? ";
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::countRecordsByTypeAllEquivalents: Failed to bind record_type. "
    ...
    rc = sqlite3_step(stmt.get());
    if (rc != SQLITE_ROW) {
        throw IOError("HistoryStorageSQLite::countRecordsByTypeAllEquivalents: Failed to execute count query. "
                      "RecordType=" + to_string(recordType) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
vector<PaymentRecord::Shared> HistoryStorageSQLite::allPaymentRecords(
    const SerializedEquivalent equivalent,
    size_t recordsCount,
    size_t fromRecord,
    DateTime timeFrom,
    ...
    bool isHighBoundaryAmountPresent)
{
    ...
    while (result.size() < recordsCount && currentOffset < paymentRecordsCount) {
        auto paymentRecords = allPaymentRecordsBase(
                                  equivalent,
                                  kPortionRequestSize,
                                  currentOffset,
                                  timeFrom,
...
vector<PaymentRecord::Shared> HistoryStorageSQLite::paymentRecordsAllEquivalents(
    size_t recordsCount,
    size_t fromRecord,
    DateTime timeFrom,
    bool isTimeFromPresent,
...
{
    if (!isLowBoundaryAmountPresent && !isHighBoundaryAmountPresent) {
        return paymentRecordsAllEquivalentsBase(
                   recordsCount,
                   fromRecord,
                   timeFrom,
                   isTimeFromPresent,
    ...
    size_t countRecordsUnderConditions = 0;
    while (result.size() < recordsCount && currentOffset < paymentRecordsCount) {
        auto paymentRecords = paymentRecordsAllEquivalentsBase(
                                  kPortionRequestSize,
                                  currentOffset,
                                  timeFrom,
                                  isTimeFromPresent,
...
vector<PaymentAdditionalRecord::Shared> HistoryStorageSQLite::allPaymentAdditionalRecords(
    const SerializedEquivalent equivalent,
    size_t recordsCount,
    size_t fromRecord,
    DateTime timeFrom,
...
{
    if (!isLowBoundaryAmountPresent && !isHighBoundaryAmountPresent) {
        return allPaymentAdditionalRecordsBase(
                   equivalent,
                   recordsCount,
                   fromRecord,
                   timeFrom,
    ...
    size_t countRecordsUnderConditions = 0;
    while (result.size() < recordsCount && currentOffset < paymentRecordsCount) {
        auto paymentAdditionalRecords = allPaymentAdditionalRecordsBase(
                                            equivalent,
                                            kPortionRequestSize,
                                            currentOffset,
                                            timeFrom,
...
vector<PaymentAdditionalRecord::Shared> HistoryStorageSQLite::allPaymentAdditionalRecordsBase(
    const SerializedEquivalent equivalent,
    size_t recordsCount,
    size_t fromRecord,
    DateTime timeFrom,
    ...
    bool isTimeToPresent)
{
    ...
    int rc = sqlite3_prepare_v2(mDataBase, query.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::allAdditionalPaymentRecords: "
                      "Bad query; sqlite error: " + to_string(rc));
    }
    int idxParam = 1;
    rc = sqlite3_bind_int(stmt, idxParam++, equivalent);
...
vector<Record::Shared> HistoryStorageSQLite::recordsPortionWithContractor(
    vector<BaseAddress::Shared> contractorAddresses,
    size_t recordsCount,
    size_t fromRecord)
{
    vector<Record::Shared> result;
    string query = "SELECT operation_uuid, operation_timestamp, record_body, record_body_bytes_count, record_type, equivalent"
                   " FROM " + mMainTableName + " "
    ...
    int rc = sqlite3_prepare_v2(mDataBase, query.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::recordsPortionWithContractor: "
                      "Bad query; sqlite error: " + to_string(rc));
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW ) {
        ...
        auto equivalent = (SerializedEquivalent)sqlite3_column_int(stmt, 5);
        switch (recordType) {
        ...
        case Record::PaymentRecordType:
            ...
            break;
        default:
            throw ValueError("HistoryStorageSQLite::recordsPortionWithContractor: "
                             "invalid record type");
        }
    }
...
vector<Record::Shared> HistoryStorageSQLite::recordsWithContractor(
    vector<BaseAddress::Shared> contractorAddresses,
    const SerializedEquivalent equivalent,
    size_t recordsCount,
    size_t fromRecord)
{
    ...
    int rc = sqlite3_prepare_v2(mDataBase, query.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::recordsWithContractor: "
                      "Bad query; sqlite error: " + to_string(rc));
    }
...
vector<PaymentRecord::Shared> HistoryStorageSQLite::paymentRecordsByCommandUUID(
    const CommandUUID &commandUUID)
{
    ...
    int rc = sqlite3_prepare_v2(mDataBase, query.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::paymentRecordsByCommandUUID: "
                      "Bad query; sqlite error: " + to_string(rc));
    }
...
vector<PaymentRecord::Shared> HistoryStorageSQLite::paymentRecordsByTransactionUUID(
    const TransactionUUID &transactionUUID)
{
    ...
    int rc = sqlite3_prepare_v2(mDataBase, query.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::paymentRecordsByTransactionUUID: "
                      "Bad query; sqlite error: " + to_string(rc));
    }
...
bool HistoryStorageSQLite::whetherOperationWasConducted(
    const TransactionUUID &transactionUUID)
{
    ...
    if (rc != SQLITE_OK) {
        throw IOError("HistoryStorageSQLite::whetherOperationWasConducted: Failed to bind transaction_uuid. "
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
TrustLineRecord::Shared HistoryStorageSQLite::deserializeTrustLineRecord(
    sqlite3_stmt *stmt)
{
...
PaymentRecord::Shared HistoryStorageSQLite::deserializePaymentRecord(
    sqlite3_stmt *stmt)
{
    TransactionUUID operationUUID((uint8_t*)sqlite3_column_blob(stmt, 0));
    auto timestamp = (GEOEpochTimestamp)sqlite3_column_int64(stmt, 1);
...
PaymentAdditionalRecord::Shared HistoryStorageSQLite::deserializePaymentAdditionalRecord(
    sqlite3_stmt *stmt)
{
    TransactionUUID operationUUID((uint8_t*)sqlite3_column_blob(stmt, 0));
    auto timestamp = (GEOEpochTimestamp)sqlite3_column_int64(stmt, 1);
...
LoggerStream HistoryStorageSQLite::info() const
{
    return mLog.info(logHeader());
}

LoggerStream HistoryStorageSQLite::debug() const
{
    return mLog.debug(logHeader());
}

LoggerStream HistoryStorageSQLite::warning() const
{
    return mLog.warning(logHeader());
}

const string HistoryStorageSQLite::logHeader() const
{
    stringstream s;
    s << "[HistoryStorage]";
    return s.str();
