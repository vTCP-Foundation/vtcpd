    ...
    mLog(logger)
{
    ...
    if (dbConnection == nullptr) {
        throw ValueError("OutgoingPaymentReceiptHandlerSQLite::constructor: Database connection cannot be null.");
    }

    if (tableName.empty()) {
        throw ValueError("OutgoingPaymentReceiptHandlerSQLite::constructor: Table name cannot be empty.");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::constructor: Failed to create table '" + mTableName + "'. "
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::constructor: Failed to create transaction_uuid index on table '" + mTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
void OutgoingPaymentReceiptHandlerSQLite::saveRecord(
...
{
    if (!ownPublicKeyHash) {
        throw ValueError("OutgoingPaymentReceiptHandlerSQLite::saveRecord: Own public key hash cannot be null.");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::saveRecord: Failed to bind trust_line_id. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::saveRecord: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::saveRecord: Failed to bind transaction_uuid. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::saveRecord: Failed to bind own_public_key_hash. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::saveRecord: Failed to bind amount. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::saveRecord: Failed to execute INSERT. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
map<TransactionUUID, TrustLineAmount> OutgoingPaymentReceiptHandlerSQLite::auditAmounts(
    const TrustLineID trustLineID,
    const AuditNumber auditNumber)
{
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::auditAmounts: Failed to bind trust_line_id. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 2, auditNumber);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::auditAmounts: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
vector<ReceiptRecord::Shared> OutgoingPaymentReceiptHandlerSQLite::receiptsByAuditNumber(
    const TrustLineID trustLineID,
    const AuditNumber auditNumber)
{
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::receiptsByAuditNumber: Failed to bind trust_line_id. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 2, auditNumber);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::receiptsByAuditNumber: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
vector<ReceiptRecord::Shared> OutgoingPaymentReceiptHandlerSQLite::receiptsLessEqualThanAuditNumber(
    const TrustLineID trustLineID,
    const AuditNumber auditNumber)
{
    vector<ReceiptRecord::Shared> result;
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::receiptsLessEqualThanAuditNumber: Failed to bind trust_line_id. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 2, auditNumber);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::receiptsLessEqualThanAuditNumber: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    const AuditNumber auditNumber)
{
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::countReceiptsByNumber: Failed to bind trust_line_id. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 2, auditNumber);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::countReceiptsByNumber: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc == SQLITE_ROW) {
        ...
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::countReceiptsByNumber: Failed to execute COUNT query. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
}

void OutgoingPaymentReceiptHandlerSQLite::deleteRecords(
    const TransactionUUID &transactionUUID)
{
    ...
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::deleteRecords: Failed to bind transaction_uuid. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }

    rc = sqlite3_step(stmt.get());
    if (rc != SQLITE_DONE) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::deleteRecords: Failed to execute DELETE by transaction. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    const TrustLineID trustLineID)
{
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::deleteRecords: Failed to bind trust_line_id. "
                      "TrustLine=" + to_string(trustLineID) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_step(stmt.get());
    if (rc != SQLITE_DONE) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::deleteRecords: Failed to execute DELETE by trust line. "
                      "TrustLine=" + to_string(trustLineID) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
void OutgoingPaymentReceiptHandlerSQLite::deleteRecordsByAuditNumber(
    const TrustLineID trustLineID,
    const AuditNumber auditNumber)
{
    ...
    int rc = sqlite3_bind_int(stmt.get(), 1, trustLineID);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::deleteRecordsByAuditNumber: Failed to bind trust_line_id. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_bind_int(stmt.get(), 2, auditNumber);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::deleteRecordsByAuditNumber: Failed to bind audit_number. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    rc = sqlite3_step(stmt.get());
    if (rc != SQLITE_DONE) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::deleteRecordsByAuditNumber: Failed to execute DELETE. "
                      "TrustLine=" + to_string(trustLineID) + ", AuditNumber=" + to_string(auditNumber) +
                      ". SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
bool OutgoingPaymentReceiptHandlerSQLite::isContainsKeyHash(
    KeyHash::Shared keyHash)
{
    if (!keyHash) {
        throw ValueError("OutgoingPaymentReceiptHandlerSQLite::isContainsKeyHash: Key hash cannot be null.");
    }
    ...
    int rc = sqlite3_bind_blob(stmt.get(), 1, keyHash->data(),
                               (int)KeyHash::kBytesSize, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::isContainsKeyHash: Failed to bind own_public_key_hash. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
bool OutgoingPaymentReceiptHandlerSQLite::isContainsTransaction(
    const TransactionUUID &transactionUUID)
{
    ...
    int rc = sqlite3_bind_blob(stmt.get(), 1, transactionUUID.data, TransactionUUID::kBytesSize, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("OutgoingPaymentReceiptHandlerSQLite::isContainsTransaction: Failed to bind transaction_uuid. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
LoggerStream OutgoingPaymentReceiptHandlerSQLite::info() const
{
    return mLog.info(logHeader());
}

LoggerStream OutgoingPaymentReceiptHandlerSQLite::warning() const
{
    return mLog.warning(logHeader());
}

const string OutgoingPaymentReceiptHandlerSQLite::logHeader() const
{
    stringstream s;
