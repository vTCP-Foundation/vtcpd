...
CommunicatorMessagesQueueHandlerSQLite::CommunicatorMessagesQueueHandlerSQLite(
    ...
    mLog(logger)
{
    ...
    if (rc != SQLITE_OK) {
        throw IOError("CommunicatorMessagesQueueHandlerSQLite::creating table: "
...
void CommunicatorMessagesQueueHandlerSQLite::saveRecord(
    ...
    size_t messageBytesCount)
{
    ...
    int rc = sqlite3_prepare_v2(mDataBase, query.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        throw IOError("CommunicatorMessagesQueueHandlerSQLite::insert: "
                      "Bad query; sqlite error: " + to_string(rc));
    }
...
void CommunicatorMessagesQueueHandlerSQLite::deleteRecord(
    ContractorID contractorID,
    const SerializedEquivalent equivalent,
    const Message::SerializedType messageType)
{
    ...
    int rc = sqlite3_prepare_v2(mDataBase, query.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        throw IOError("CommunicatorMessagesQueueHandlerSQLite::delete: "
                      "Bad query; sqlite error: " + to_string(rc));
    }
    rc = sqlite3_bind_int(stmt, 1, contractorID);
    if (rc != SQLITE_OK) {
...
vector<tuple<ContractorID, BytesShared, Message::SerializedType>> CommunicatorMessagesQueueHandlerSQLite::allMessages()
{
    string queryCount = "SELECT count(*) FROM " + mTableName;
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(mDataBase, queryCount.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        throw IOError("CommunicatorMessagesQueueHandlerSQLite::allMessages: "
...
LoggerStream CommunicatorMessagesQueueHandlerSQLite::info() const
{
    return mLog.info(logHeader());
}

LoggerStream CommunicatorMessagesQueueHandlerSQLite::warning() const
{
    return mLog.warning(logHeader());
}

const string CommunicatorMessagesQueueHandlerSQLite::logHeader() const
{
    stringstream s;
