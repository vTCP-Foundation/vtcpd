...
FeaturesHandlerSQLite::FeaturesHandlerSQLite(
    ...
    mLog(logger)
{
    ...
    if (dbConnection == nullptr) {
        throw ValueError("FeaturesHandlerSQLite::constructor: Database connection cannot be null.");
    }

    if (tableName.empty()) {
        throw ValueError("FeaturesHandlerSQLite::constructor: Table name cannot be empty.");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("FeaturesHandlerSQLite::constructor: Failed to create table '" + mTableName + "'. "
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("FeaturesHandlerSQLite::constructor: Failed to create unique index on table '" + mTableName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
void FeaturesHandlerSQLite::saveFeature(
    const string &featureName,
    const string &featureValue)
{
    if (featureName.empty()) {
        throw ValueError("FeaturesHandlerSQLite::saveFeature: Feature name cannot be empty.");
    }
    ...
    if (rc != SQLITE_OK) {
        throw IOError("FeaturesHandlerSQLite::saveFeature: Failed to bind feature_name. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("FeaturesHandlerSQLite::saveFeature: Failed to bind feature_length. "
    ...
    if (rc != SQLITE_OK) {
        throw IOError("FeaturesHandlerSQLite::saveFeature: Failed to bind feature_value. "
                      "FeatureName='" + featureName + "', FeatureLength=" + to_string(featureValue.size()) + ". "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc != SQLITE_DONE) {
        throw IOError("FeaturesHandlerSQLite::saveFeature: Failed to execute INSERT OR REPLACE. "
                      "FeatureName='" + featureName + "', FeatureLength=" + to_string(featureValue.size()) + ". "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
...
string FeaturesHandlerSQLite::getFeature(
    const string &featureName)
{
    if (featureName.empty()) {
        throw ValueError("FeaturesHandlerSQLite::getFeature: Feature name cannot be empty.");
    }
    ...
    int rc = sqlite3_bind_text(stmt.get(), 1, featureName.c_str(), (int)featureName.size(), SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        throw IOError("FeaturesHandlerSQLite::getFeature: Failed to bind feature_name. "
                      "FeatureName='" + featureName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
    ...
    if (rc == SQLITE_ROW) {
        ...
        if (featureValueBytes == nullptr && featureSize > 0) {
            throw IOError("FeaturesHandlerSQLite::getFeature: Retrieved null feature value with non-zero size. "
        ...
        throw NotFoundError("FeaturesHandlerSQLite::getFeature: Feature not found. "
                            "FeatureName='" + featureName + "'.");
    } else {
        throw IOError("FeaturesHandlerSQLite::getFeature: Failed to execute SELECT. "
                      "FeatureName='" + featureName + "'. "
                      "SQLite error: " + to_string(rc) + " (" + sqlite3_errmsg(mDataBase) + ").");
    }
}

LoggerStream FeaturesHandlerSQLite::info() const
{
    return mLog.info(logHeader());
}

LoggerStream FeaturesHandlerSQLite::warning() const
{
    return mLog.warning(logHeader());
}

const string FeaturesHandlerSQLite::logHeader() const
{
    stringstream s;
